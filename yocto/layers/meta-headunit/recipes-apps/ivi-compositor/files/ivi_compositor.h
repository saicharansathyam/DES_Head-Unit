...
public:
    ...
    // NEW: allow focusing by app name from QML (optional convenience)
    Q_INVOKABLE void focusApp(const QString &name);
...
private:
    ...
    // NEW: central helper to grant input focus so buttons get events
    void grantFocus(QWaylandQuickSurface *surface);
};
