...
void ApplicationFrameworkManager::launchInitialApplications()
{
    ...
    // Check if compositor is ready
    if (!isWaylandCompositorReady()) {
        logWarning("Compositor not ready, retrying in 2 seconds...");

        QTimer::singleShot(2000, this, &ApplicationFrameworkManager::launchInitialApplications);
        return;
    }

    logInfo("Compositor is ready, proceeding with app launches");
...
QProcessEnvironment ApplicationFrameworkManager::createAppEnvironment(int iviId)
{
    ...
    // Get XDG_RUNTIME_DIR (critical for Wayland)
    QString xdgRuntime = env.value("XDG_RUNTIME_DIR");
    if (xdgRuntime.isEmpty()) {
        xdgRuntime = "/tmp";
        logWarning("XDG_RUNTIME_DIR not set in environment, using /tmp");
    }

    // Verify Wayland socket exists
    QString waylandSocket = QDir(xdgRuntime).filePath("wayland-1");
    if (!QFile::exists(waylandSocket)) {
        logError(QString("Wayland socket not found: %1").arg(waylandSocket));
        logError("Application will likely fail to start!");
    } else {
        logInfo(QString("Wayland socket found: %1").arg(waylandSocket));
    }
    ...
    // Enable debugging for troubleshooting (disable in production)
